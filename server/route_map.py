...
def render_route_map(obs: dict, scenario_shipments: list | None = None) -> str:
    ...
    Returns an HTML string with inline SVG that works in Gradio's gr.HTML.
    """
    ...
    # Draw background network edges (nearby city pairs)
    ...
    svg_parts.append(
        ...
        f'text-transform="uppercase">INDIAN LOGISTICS NETWORK</text>'
    )

    svg_parts.append("</svg>")

    # Build legend + status cards below the map
    legend_items = []
    for ship in ships:
        color = _STATUS_COLORS.get(ship["status"], "#7a8ea0")
